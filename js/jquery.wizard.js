	...
	methods = {
		init: function (element, options) {
			...
			if(opts.topButtons && stepsBar.length && !topActions.length){
				...
				html += '<span class="btn-prev prev button"><span class="previous-text">'+ opts.text.previous +'</span></span>';
				html += '<span class="btn-next next button"><span class="next-text">'+ opts.text.next +'</span><span class="finished-text">'+ opts.text.finished +'</span></span>';
				html += '</div></div>';
			...
			if(opts.bottomButtons && !bottomActions.length){
				...
				html += '<div class="left-actions"><span class=" btn-prev button prev"><span class="previous-text">'+ opts.text.previous +'</span></span></div>';
				html += '<div class="right-actions"><span class=" btn-next button next"><span class="next-text">'+ opts.text.next +'</span><span class="finished-text">'+ opts.text.finished +'</span></span></div>';
				html += '</div>';
			...
			for(var i=0;i<stepsLi.length;i++){
				...
				step.empty().html('<span class="step-index"><span class="label">'+ (i+1) +'</span></span>'+ content + '<span class="wiz-icon-chevron-right colorA"></span>');
